...
else header("Location: principal.php?methodSub=select?error=nombre");
if(is_numeric($data[1]) && $data[1] > 0)
    $data[1] = filter_var(trim($data[1]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
else header("Location: principal.php?methodSub=select?error=precio");
if(is_numeric($data[2]) && $data[2] > 0)
    $data[2] = filter_var(trim($data[2]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
else header("Location: principal.php?methodSub=select?error=meses");
?>